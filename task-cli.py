...
class Task: 
    ...
    def __init__(self, description, id=None, status="todo", createdAt=None, updateAt=None): 
        self.id = id
        self.description = description
        self.status = status
        self.created_at = createdAt if createdAt is not None else time.ctime()
        self.update_at = updateAt if updateAt is not None else time.ctime()
        ...
        if os.path.exists(self.filename): 
            if os.path.getsize(self.filename) > 0: 
                try:
                    ...
                    if raw_data:
                        Task.id_counter = max(d['id'] for d in raw_data)
